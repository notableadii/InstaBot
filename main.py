...
AUTO_SHUTDOWN_MINUTES = 30  # Auto-shutdown after X minutes (0 = disabled)
...
class InstagramCouplebotDebug:
    def __init__(self):
        ...
        if AUTO_SHUTDOWN_MINUTES > 0:
            self.shutdown_time = datetime.now() + timedelta(minutes=AUTO_SHUTDOWN_MINUTES)
            logger.info(f"⏰ Bot will auto-shutdown at: {self.shutdown_time.strftime('%H:%M:%S')}")
        else:
    ...
    def check_auto_shutdown(self):
        """Check if it's time to auto-shutdown"""
        if self.shutdown_time and datetime.now() >= self.shutdown_time:
            logger.info("⏰ AUTO-SHUTDOWN: 30 minutes elapsed")
            logger.info("🛑 Bot shutting down automatically for safety")
            self.session_stats['shutdown_time'] = datetime.now()
    ...
    def start_monitoring(self):
        ...
        if AUTO_SHUTDOWN_MINUTES > 0:
            logger.info(f"⏰ Auto-shutdown in: {AUTO_SHUTDOWN_MINUTES} minutes")
        else:
        ...
        try:
            while True:
                ...
                logger.info(f"⏳ Waiting {wait_time} seconds before next check... ({remaining_time})")
